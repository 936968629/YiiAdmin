...
<?php $this->beginPage(); ?>
        ...
        <div class="builder-toolbar builder-list-toolbar">
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-9 button-list">
                    ...
                    <a title="启用" class="btn btn-success ajax-post confirm" data-model="User" href="/index.php?s=/admin/user/setstatus/status/resume/model/User.html" autocomplete="off">启用</a>&nbsp;
                    <a title="禁用" class="btn btn-warning ajax-post confirm" data-model="User" href="/index.php?s=/admin/user/setstatus/status/forbid/model/User.html">禁用</a>
                </div>
            </div>
        </div>
            ...
            <table class="table table-hover table-header" id="list-table">
                ...
                <?php foreach ($datalist as $item): ?>
                    <tr>
                        ...
                        <td><img src="<?= Yii::$app->params['apiUrl'].$item['topic_img'] ?>"></td>
                        <td>
    ...
    <script>
        function editStatus(id,status=1) {
            if(parseInt(id) ){
                $.get(common_ops.buildAdminUrl('/category/edit_status',{'id':id,'status':status}),function (data) {
